...
class SyllabusService {
  ...
  async getSyllabusByStatus(
    status: "pending" | "approved" | "rejected"
  ): Promise<ApiResponse<Syllabus[]>> {
    return fetch(this.SYLLABUS_BASE_URL + `?status=${status}`).then((res) =>
      res.json()
    );
  }
  ...
  async getSyllabusByAuthorUID(
    authorUID: string
  ): Promise<ApiResponse<Syllabus[]>> {
    return fetch(this.SYLLABUS_BASE_URL + `?authorUID=${authorUID}`).then(
      (res) => res.json()
    );
  }
}
